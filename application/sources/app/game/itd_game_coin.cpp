...
static bool check_spawn_overlap(uint8_t x, uint8_t y, uint8_t w, uint8_t h)
{
    for (uint8_t i = 0; i < GIFT_NUMBER_MAX; i++)
    {
        ...
        {
            if ((x + w > gifts[i].x) && (x < gifts[i].x + GIFT_SIZE_BITMAP_X) && (y + h > gifts[i].y) &&
                (y < gifts[i].y + GIFT_SIZE_BITMAP_Y))
            {
                return true;
            }
        }
    ...
    for (uint8_t i = 0; i < COIN_NUMBER_MAX; i++)
    {
        ...
        {
            if ((x + w > coins[i].x) && (x < coins[i].x + COIN_SIZE_BITMAP_X) && (y + h > coins[i].y) &&
                (y < coins[i].y + COIN_SIZE_BITMAP_Y))
            {
                return true;
            }
        }
    ...
    for (uint8_t i = 0; i < SPIKE_NUMBER; i++)
    {
        if (spikes[i].visible == WHITE)
        {
            ...
            if ((x + w > spikes[i].x) && (x < spikes[i].x + spike_w) && (y + h > spikes[i].y) &&
                (y < spikes[i].y + spike_h))
                return true;
        }
    }
        ...
        case ITD_GAME_COIN_SPAWN:
        {
            ...
            else
            {
                for (uint8_t i = 0; i < coin_number; i++)
                {
                    ...
                    do
                    {
                        temp_x = COIN_SPAWN_AXIS_X_MIN + rand() % (COIN_SPAWN_AXIS_X_MAX - COIN_SPAWN_AXIS_X_MIN) +
                                 COIN_SPAWN_LEFT_OFFSET;
                        temp_y = COIN_SPAWN_AXIS_Y_MIN + rand() % (COIN_SPAWN_AXIS_Y_MAX - COIN_SPAWN_AXIS_Y_MIN) +
                                 COIN_SPAWN_TOP_OFFSET;
