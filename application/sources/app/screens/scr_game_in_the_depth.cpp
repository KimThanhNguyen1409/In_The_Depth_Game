...
void itd_game_bomb_display()
{
    for (uint8_t i = 0; i < BOMB_NUMBER_MAX; i++)
    {
        if (bombs[i].visible != WHITE)
            continue;
        const unsigned char* frames[4] = {bomb_I, bomb_II, bomb_III, bomb_IV};
        uint8_t w                      = BOMB_SIZE_BITMAP_X;
        uint8_t h                      = BOMB_SIZE_BITMAP_Y;
        view_render.drawBitmap(bombs[i].x, bombs[i].y, frames[bombs[i].action_image], w, h, WHITE);
    }
}
    ...
    for (uint8_t i = 0; i < SPIKE_NUMBER; i++)
    {
        if (spikes[i].type == 2)
        {
            ...
            view_render.drawBitmap(
                spikes[i].x, spikes[i].y, spike_tall, SPIKE_TALL_SIZE_BITMAP_X, SPIKE_TALL_SIZE_BITMAP_Y, WHITE);
        }
        else
        {
            if (spikes[i].visible != WHITE)
                continue;
            view_render.drawBitmap(
                spikes[i].x, spikes[i].y, spike_short, SPIKE_SHORT_SIZE_BITMAP_X, SPIKE_SHORT_SIZE_BITMAP_Y, WHITE);
        }
    }
}
...
void itd_game_boom_display()
{
    for (uint8_t i = 0; i < BOOM_NUMBER; i++)
    {
        ...
        view_render.drawBitmap(boom[i].x, boom[i].y, frames[boom[i].action_image], w, h, WHITE);
    }
}
...
void scr_game_in_the_depth_handle(ak_msg_t* msg)
{
    switch (msg->sig)
    {
        case SCREEN_ENTRY:
        {
            ...
            timer_set(AC_TASK_DISPLAY_ID, ITD_GAME_TIME_TICK, ITD_GAME_TIME_TICK_INTERVAL, TIMER_PERIODIC);
        }
        break;
        ...
        case ITD_GAME_RESET:
        {
            ...
            if (itd_game_heart == 0)
            {
                ...
                timer_set(AC_TASK_DISPLAY_ID, ITD_GAME_EXIT_GAME, ITD_GAME_TIME_EXIT_INTERVAL, TIMER_ONE_SHOT);
            }
        }
        break;
